...
// the old broken code has <XAxis right after </tbody>
// But wait, in the old file, XAxis is NOT closed. Let's see.
const oldBrokenHTMLStart = code.indexOf('<XAxis', startIdx);
const endAnchor = `                <Tooltip formatter={(value: any) => formatCurrency(value || 0)} />
...
let endIdx = code.indexOf(endAnchor, startIdx);
if (endIdx === -1) {
  // If we can't find it exactly, just search for `        <div className="glass-card">\n          <h2 className="chart-title">先月の振り返り`
  const fallbackEnd = code.indexOf('<div className="glass-card">\n          <h2 className="chart-title">先月の振り返り');
  if (fallbackEnd !== -1) {
    code = code.substring(0, startIdx) + '\n' + missingHTML + '\n\n        ' + code.substring(fallbackEnd);
    fs.writeFileSync('app/page.tsx', code);
    console.log('Fixed page.tsx using fallback end!');
  } else {
    console.log('Failed to find fallback end.');
  }
} else {
  code = code.substring(0, startIdx) + '\n' + missingHTML + '\n\n' + code.substring(endIdx + endAnchor.length);
  fs.writeFileSync('app/page.tsx', code);
